...
perf_chart_1 <- 
  AI_Funds %>% 
  ggplot() + 
  geom_line(aes(date, AI_Fund, color = "Blue"), size = 1, alpha = 0.7) + 
...
# Finplot now adds finishing touches easily:
  ...
  fmxdat::finplot(perf_chart_1, x.vert = T, x.date.type = "%Y", x.date.dist = "2 years")
```

```{r}

threshold_date <- as.Date("2003-10-31")
AI_xts <- AI_Funds %>% tbl_xts()
ASISA_xts <- ASISA %>% select(-Index, -FoF) %>% spread(Fund, Returns) 

ASISA_median <- ASISA %>% group_by(date) %>% summarise(mean_returns = mean(Returns, na.rm = TRUE), .groups = "drop") %>% tbl_xts()

BM_xts <- BM %>% select(-Tickers) %>% tbl_xts()

merged <- merge.xts(AI_xts, ASISA_median, BM_xts) %>% na.omit()

chart.CumReturns(AI_xts)
chart.CumReturns(ASISA_median)
...
chart.Histogram(merged)
library(hrbrthemes)
df_merged <- data.frame(date = index(merged), coredata(merged)) %>% gather(Funds, Returns, -date)
means <- df_merged %>%
  group_by(Funds) %>%
  summarise(mean_return = mean(Returns, na.rm = TRUE))

ggplot(data = df_merged, aes(x = Returns, group = Funds, fill = Funds)) +
...
```
```{r}
year_roll <- roll_mean(merged, 36)
df_roll <- data.frame(date = index(year_roll), coredata(year_roll)) %>% gather(Funds, Returns, -date)
means <- df_roll %>%
  group_by(Funds) %>%
  summarise(mean_return = mean(Returns, na.rm = TRUE))
...
Cumulative returns AI_Fund

```{r}
AI_Funds <- cum_returns(AI_Funds, date, AI_Fund)
AI_roll <- roll_returns(AI_Funds, date, Returns, 36)
...
ASISA_c <- ASISA %>%
  ...
  summarise(Average_Returns = mean(Returns, na.rm = TRUE)) %>% ungroup()# Calculate average returns

ASISA_roll <- roll_returns(ASISA_c, date, Average_Returns, 36)

BM_roll <- roll_returns(BM, date, Returns, 36)
...
merged_data <- BM_roll %>%
  full_join(ASISA_roll, by = "date", suffix = c("_SWIX", "_ASISA")) #%>%
  #full_join(AI_roll, by = "date", suffix = c("", "_AI"))  # Use "" for already renamed columns

```


```{r}
long <- merged_data %>% select(date, Rolling_Returns_SWIX, Rolling_Returns_ASISA) %>% 
...
g <-
long %>%
    ggplot() + geom_line(aes(date, Return, color = Index),
