...
hedged <- Return_hedged %>% select(date, Portfolio_Return_Hedged)%>%
    mutate(date = as.yearmon(date))
merged <- left_join(unhedged, hedged, by = "date") %>% tbl2xts::tbl_xts()
...
chart.CumReturns(merged)
density_plot(merged)
Return.annualized(merged)
chart.RollingPerformance(merged, legend.loc = 3)
lm(merged$Portfolio_Return_Hedged~cur_ret$Return_cur)
```
