...
Look at performance of different caps

```{r}
ALSI_index <- ALSI %>% select(date, Return, Index_Name) %>% 
    group_by(date, Index_Name) %>%
  summarise(Average_Return = mean(Return, na.rm = TRUE)) %>%
  arrange(date) %>% na.omit() %>% spread(Index_Name, Average_Return)
...
ALSI_index <- ALSI %>%
  ...
  arrange(date) %>%
  na.omit() %>%
  spread(Index_Name, Average_Return) %>% tbl_xts()

library(PerformanceAnalytics)
chart.CumReturns(ALSI_index, legend.loc = 1)


```

Look at distribution of sectors of SWIX and ALSI, we can see that ALSI lots of 
resources
...
Lets look at stock concentration, we can see that the stock concentration is
significantly higher for ALSI
...
The Capped_funds function allows us to see the impact of varying caps on different
Funds.
