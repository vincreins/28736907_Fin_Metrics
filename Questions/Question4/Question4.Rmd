...
PerformanceAnalytics::TrackingError(Ra, Rs, scale = 12)
...
chart.Histogram(xts_port, methods = "add.normal", 
                main = "Sanlam Histogram")

chart.Histogram(xts_port, 
...
```{r}
table.SFM(Ra = xts_port, Rb = xts_bench)
```
...
merged_data %>% group_by(Sector, date) 
...
ggplot(sector_weights, aes(x = date, y = total_weight, fill = Sector)) +
  ...
  labs(title = "Total Weight per Sector by Date",
       ...
       y = "Total Weight") +
  theme_minimal() +
  scale_fill_brewer(palette = "Set3") # Optional: adjust the color palette
