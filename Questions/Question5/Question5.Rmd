...
ZAR <- cncy %>% spread(Name, Price) %>% select(date, SouthAfrica_Cncy) %>% na.omit 
Rest <- cncy %>% spread(Name, Price) 
merged <- left_join(ZAR, Rest, by = "date") %>% select_if(~ !any(is.na(.))) %>% 
    tbl_xts()
    

variance <- roll_var(xts_rtn, 252) 
rolling_variance <- rollapply(xts_rtn, width = 252, FUN = var, fill = NA, align = "right") %>% 
    na.omit() %>% 
    data.frame(date = index(rolling_variance), RollingVariance = coredata(rolling_variance)) %>% 
    gather(Name, Price, -date)

# Plot
ggplot(rolling_variance, aes(x = date, y = Price, colour = Name)) +
  geom_line() 
...
stddev <- StdDev.annualized(xts_rtn, 252)

charts.TimeSeries(xts_rtn, legend.loc = 1)
